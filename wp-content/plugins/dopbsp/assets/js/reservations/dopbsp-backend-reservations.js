...
function dopbspInitReservations() {
    if (clearReservationsClick) {
        ...
        $jDOPBSP.post(ajaxurl, {
            action: 'dopbsp_init_reservations_front',
            calendar_id: $jDOPBSP('#calendar_id').val(),
            view: $jDOPBSP('#DOPBSP-reservations-view').val()
        }, function (data) {
            data = $jDOPBSP.parseJSON($jDOPBSP.trim(data));
            var hoursEnabled = data['HoursEnabled'] == 'true' ? true : false, hoursAMPM = data['HoursAMPM'] == 'true' ? true : false, paymentMethodArrivalEnabled = data['PaymentMethodArrivalEnabled'] == 'true' ? true : false, paymentMethodPayPalEnabled = data['PaymentMethodPayPalEnabled'] == 'true' ? true : false, hours = new Array('00:00', '01:00', '02:00', '03:00', '04:00', '05:00', '06:00', '07:00', '08:00', '09:00', '10:00', '11:00', '12:00', '13:00', '14:00', '15:00', '16:00', '17:00', '18:00', '19:00', '20:00', '21:00', '22:00', '23:00', '24:00'), hoursHTML = new Array(), i;
...
function dopbspInitListReservations(data) {
    ...
    $jDOPBSP('#DOPBSP-reservations-start-day').datepicker({
        minDate: 0,
    ...
    $jDOPBSP('#DOPBSP-reservations-end-day').datepicker({
        minDate: 0,
        dateFormat: parseInt(data['DateType'], 10) == 1 ? 'MM dd, yy' : 'dd MM yy',
    ...
    $jDOPBSP('#DOPBSP-reservations-start-day').bind('change', function () {
        var year = parseInt($jDOPBSP('#DOPBSP-reservations-start-day-alt').val().split('-')[0], 10), month = parseInt($jDOPBSP('#DOPBSP-reservations-start-day-alt').val().split('-')[1], 10) - 1, day = parseInt($jDOPBSP('#DOPBSP-reservations-start-day-alt').val().split('-')[2], 10), minDateValue = dopbspDateDiference(new Date(), new Date(year, month, day));
        $jDOPBSP('#DOPBSP-reservations-end-day').datepicker('destroy');
        $jDOPBSP('#DOPBSP-reservations-end-day').datepicker({
...
function dopbspSetSearchReservations(data) {
    var i, searchHTML = new Array(), form = data['Form'];
...
function dopbspSetNoPagesReservations() {
    var i, pagesHTML = new Array(), noPages = Math.ceil(parseInt($jDOPBSP('#DOPBSP-reservations-no-pages').val()) / parseInt($jDOPBSP('#DOPBSP-reservations-pagination-no-page').val()));
    for (i = 1; i <= noPages; i++) {
...
function dopbspGetListReservations(stopAJAX, resetPages) {
    dopbspToggleReservationsMessage('show', DOPBSP_LOAD);
    ...
    ajaxRequestInProgress = $jDOPBSP.post(ajaxurl, {
        action: 'dopbsp_get_list_reservations_front',
        calendar_id: $jDOPBSP('#calendar_id').val(),
        ...
        order_by: $jDOPBSP('#DOPBSP-reservations-order-by').val()
    }, function (data) {
...
function dopbspInitApproveReservation() {
    ...
    $jDOPBSP('.DOPBSP-reservations-approve-button').bind('click', function () {
        var id = $jDOPBSP(this).attr('id').split('DOPBSP-reservations-approve-button')[1], wasPending = $jDOPBSP('#DOPBSP_Reservation_ID' + id + ' .flag').hasClass('pending') ? true : false, noReservations = 0;
        if (confirm(DOPBSP_RESERVATIONS_APPROVE_CONFIRMATION)) {
            ...
            $jDOPBSP.post(ajaxurl, {
                action: 'dopbsp_approve_reservation_front',
                calendar_id: $jDOPBSP('#calendar_id').val(),
                reservation_id: id
            }, function (data) {
                if (wasPending) {
...
function dopbspInitRejectReservation() {
    ...
    $jDOPBSP('.DOPBSP-reservations-reject-button').bind('click', function () {
        var id = $jDOPBSP(this).attr('id').split('DOPBSP-reservations-reject-button')[1], wasPending = $jDOPBSP('#DOPBSP_Reservation_ID' + id + ' .flag').hasClass('pending') ? true : false, noReservations = 0;
        if (confirm(DOPBSP_RESERVATIONS_REJECT_CONFIRMATION)) {
            ...
            dopbspToggleReservationsMessage('show', DOPBSP_SAVE);
            $jDOPBSP.post(ajaxurl, {
                action: 'dopbsp_reject_reservation_front',
                calendar_id: $jDOPBSP('#calendar_id').val(),
                reservation_id: id
            }, function (data) {
                if (wasPending) {
...
function dopbspInitCancelReservation() {
    ...
    $jDOPBSP('.DOPBSP-reservations-cancel-button').bind('click', function () {
        ...
        if (confirm(DOPBSP_RESERVATIONS_CANCEL_CONFIRMATION)) {
            ...
            dopbspToggleReservationsMessage('show', DOPBSP_SAVE);
            $jDOPBSP.post(ajaxurl, {
                action: 'dopbsp_cancel_reservation_front',
                calendar_id: $jDOPBSP('#calendar_id').val(),
                reservation_id: id
            }, function (data) {
                if ($jDOPBSP('#calendar_refresh').val() != undefined) {
...
function dopbspInitDeleteReservation() {
    ...
    $jDOPBSP('.DOPBSP-reservations-delete-button').bind('click', function () {
        ...
        if (confirm(DOPBSP_RESERVATIONS_DELETE_CONFIRMATION)) {
            dopbspToggleReservationsMessage('show', DOPBSP_SAVE);
            $jDOPBSP.post(ajaxurl, {action: 'dopbsp_delete_reservation_front', reservation_id: id}, function (data) {
...
function dopbspInitAddReservation() {
    if (clearReservationsClick) {
        dopbspToggleReservationsMessage('show', DOPBSP_LOAD);
        $jDOPBSP('#DOPBookingSystemPROReservations_Info' + $jDOPBSP('#calendar_id').val()).remove();
        $jDOPBSP('.DOPBSP-admin .reservations-content').html('');
        ...
        $jDOPBSP('#DOPBSP-reservations-order-filter').css('display', 'none');
        $jDOPBSP.post(ajaxurl, {
            action: 'dopbsp_init_add_reservation_front',
            calendar_id: $jDOPBSP('#calendar_id').val()
        }, function (data) {
            data = $jDOPBSP.parseJSON($jDOPBSP.trim(data));
...
function dopbspToggleReservationsMessage(action, message) {
    if (action == 'show') {
