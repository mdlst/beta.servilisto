 ...
 */
    ...
    if (!class_exists('DOTModelAvailability')){
		 ...
		 * Initialize tables.
		 */
		array_push($query, 'SELECT DISTINCT availability_price.price');
		array_push($query, ' FROM '.$DOT->tables->availability_price.' availability_price ');
	    
                
                if ($day_start != ''
			&& $day_end != ''){
		    array_push($query, ' WHERE availability_price.calendar_id =%s AND (availability_price.date_start>=%s AND availability_price.date_start<=%s OR availability_price.date_end>=%s AND availability_price.date_end<=%s OR availability_price.date_start>=%s AND availability_price.date_end<=%s OR availability_price.date_start<=%s AND availability_price.date_end>=%s)');
		    array_push($values, $calendar_id, $date_start, $date_end, $date_start, $date_end, $date_start, $date_end, $date_start, $date_end);
                        }
                else if ($day_start != '' 
                            && $day_end == ''){
                    $day_end = $day_start;
		 ...
		 * Execute query.
		 */
		$prices = $DOT->db->results($DOT->db->safe(implode('', $query),
							      $values));
