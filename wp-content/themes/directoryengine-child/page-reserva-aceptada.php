...
if (isset($_GET["acceptres"]) && $_GET["acceptres"]) {
    $decodido = base64_decode($_GET["acceptres"]);
    $decode_explode = explode("&", $decodido);

    if (sizeof($decode_explode) == 5 && $decode_explode[0] == "accept" && $decode_explode[4] == "pending") {
        ...
        if ($reservation->status == 'pending') {  // en
            $res = $DOPBSP->classes->backend_reservation->approve($decode_explode[1]);
            if ($res == "") $exito = 1;
        } else {
            $exito = 2;
        }
...
if ($exito == 0) {
    ...
    $mensaje = "<p>Ha habido un problema con la aceptación de la reserva.</p>
                <p>Si el problema persiste, puedes intentar aceptar la reserva
                   entrando a tu
                   <a href='" . get_bloginfo('url') . "/mi-perfil" . "'>panel de usuario</a>
                   dentro de la zona de administración de Servilisto.com</p>";
} elseif ($exito == 1) {
    $title = "Reserva aceptada";
    $mensaje = "<p>La reserva ha sido aceptada correctamente.</p>
                <p>No olvides de en caso de cancelación deberás entrar en tu
                <a href='" . get_bloginfo('url') . "/mi-perfil" . "'>panel de usuario</a> con el
                   tiempo necesario para evitar comentarios negativos.</p>";
} else {
