...
if ($events->have_posts()) {
                ...
                <div class="event-wrapper event-item">
                    <div class="event-wrapper" itemscope itemtype="http://schema.org/Event">
                        ...
                        <h3 class="title-envent">
                            ...
                            <span class="ribbon-event"><span
                                    class="ribbon-event-content"><?php echo $event->ribbon; ?></span></span>
                            <?php if (ae_user_can('edit_others_posts') || $user_ID == $post->post_author) { ?>
                                <ol class="edit-event-option">
                                    ...
                                    <?php if ($post->post_status === 'pending' && ae_user_can('edit_others_posts')) { ?>
                                        <li style="display:inline-block"><a href="#" class="action approve"
                                                                            data-action="approve"><i
                                                    class="fa fa-check"></i></a></li>
                                    <?php }
                                    ?>
                                    <li style="display:inline-block"><a href="#" class="action edit" data-action="edit"><i
                                                class="fa fa-pencil"></i></a></li>
                                    <?php if ($post->post_status == 'publish') { ?>
                                        <li style="display:inline-block"><a href="#" class="action archive"
                                                                            data-action="archive"><i
                        ...
                        <time>
                            <span itemprop="startDate"
                                  content="<?php echo date('Y-m-d H:i:s', strtotime($event->open_time)); ?>"></span>
                            <span itemprop="endDate"
                                  content="<?php echo date('Y-m-d H:i:s', strtotime($event->close_time)); ?>"></span>
    ...
    if (wp_is_mobile()) {  // en version movil, mostramos que no hay ofertas
        echo '<div class="align-centered">
                <p>Actualmente no hay ofertas</p>
                </div>';
    }
}
wp_reset_postdata();
