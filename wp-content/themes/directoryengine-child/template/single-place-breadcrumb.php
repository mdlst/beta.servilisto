...
<div class="section-detail-wrapper ">
    ...
    if (($current_user->ID == $post->post_author || current_user_can('administrator')) /*&& !$detector->isTablet()*/) { ?>
        ...
        <div class="dropdown">
            <button class="btn btn-default dropdown-toggle editar-anuncio" type="button" id="dropdownMenu1" data-toggle="dropdown">
                <span style="margin-right: 6px;" class="caret"></span>
                Editar Anuncio
            </button>

            <script type="application/javascript">
                jQuery(document).ready( function() {  /*FIX PARA ARREGLAR LOS DROPDOWN-TOGGLE*/
                    jQuery('#dropdownMenu1.dropdown-toggle').dropdown();
                });
            </script>


            <ul style="text-align:right;" class="dropdown-menu dropdown-menu-right single-place-control" role="menu"
                aria-labelledby="dropdownMenu1">
                <li role="presentation">
                    <a class="place-action edit" role="menuitem" data-action="edit" href="#edit_place"
                       data-target="#edit_place">
                        <i class="fa fa-pencil"></i> <?php _e("Editar anuncio", ET_DOMAIN); ?>
                    </a>
                </li>
                <li role="presentation">
                    <a class="place-action edit_calender edit" role="menuitem" data-action="edit_calender"
                       href="#edit_calender" data-target="#edit_calender">
                ...
                <li role="presentation">
                    ...
                    <a class="place-action create_event edit oculto" data-toggle="modal" data-action="create_event" href="#">
                ...
                <?php if (current_user_can('administrator')) {
                    ...
                    <li role="presentation">
                        <a <?php if ($place->et_featured) {
                            echo 'style="color:#E89825;"';
                        } ?> class="place-action" role="menuitem" data-action="toggleFeature" href="#">
                ...
                if ($place->et_claimable && !$place->et_claim_approve && !empty($et_claim_info) && current_user_can('manage_options')) {
                    ?>
                    <li role="presentation">
                        <a href="#" data-id="<?php echo $place->ID ?>" class="<?php if ($user_ID) {
                            echo 'claim-place';
                        } else {
                            echo 'authenticate';
                        } ?>">
                ...
                <?php if (!isset($place->et_payment_package) || $place->et_payment_package == '' || $price == 0) { ?>
                    <li role="presentation">
                        <a class="place-action edit oculto"
                           href="<?php echo et_get_page_link('post-place', array('id' => $place->ID)) ?>">
                            <i class="fa fa-level-up"></i> <?php _e("Mejorar Anuncio", ET_DOMAIN); ?>
                        </a>
                    </li>
                <?php } ?>
                <?php
