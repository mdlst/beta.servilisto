...
*/
    ...
    if (!class_exists('DOPBSPViewsBackEndReservationsFront')){
        class DOPBSPViewsBackEndReservationsFront extends DOPBSPViewsBackEnd{
            /*
             * Constructor
             */
 ...
 */
            ...
            function listCalendars(){
				...
				$calendars_qry = $wpdb->get_results('SELECT * FROM '.$wpdb->prefix.'dopbsp_calendars WHERE user_id="'.$user_ID.'" ORDER BY id');
				$calendar_id = '';
				$calendar_ar = array();
				if(is_array($calendars_qry) and count($calendars_qry)>0){
					foreach($calendars_qry as $calendar_a){
						$calendar_ar[] = $calendar_a->id;
					}
				}
                ...
                if (count($calendars) > 0){
                    foreach ($calendars as $calendar){
						if(in_array($calendar->id,$calendar_ar)){
