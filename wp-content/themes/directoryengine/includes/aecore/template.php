...
function ae_tax_dropdown( $tax, $args = array() ) {
    ...
    if ( ! empty( $categories ) ) {
        ...
        if ( $show_option_all ) {
            ...
            $output .= "\t<option data='por-defecto' value=''$selected>$show_option_all</option>\n";
        }

        $show_our_location=false;
        if ( $show_our_location ) {

            $selected = "selected='selected";
            $output .= "\t<option value=''$selected>Mi localización</option>\n";
        }
...
class AE_Walker_TaxDropdown extends Walker_CategoryDropdown{
    ...
    function start_el( &$output, $category, $depth = 0, $args = array(), $id = 0 ) {
        ...
        if($category->taxonomy == "place_category")
        {
            $arg = array('child_of' => $category->term_id, 'taxonomy' => 'place_category');
            $categories = get_categories( $arg );
            $var = (!empty($categories)) ? 'disabled' : '';
            $attr = ' data-id="child_'.$category->term_id.'" ';;
            if($category->parent != "")
                $class = 'child-cat-'.$category->parent.' ';
        }
        /* 09_11_2016 Id : 1 S*/
        ...
        if(isset($args['value']) && $args['value'] == 'slug' ) {
            $output .= "\t<option class=\" $category->slug cat-$category->term_id ".$class."level-$depth\" value=\"".$category->slug."\"";
        } else {
            $output .= "\t<option class=\" $category->slug cat-$category->term_id ".$class."level-$depth\" value=\"".$category->term_id."\"";
        }
        /* 09_11_2016 Id : 2 E*/
        // if(isset($args['value'])  && $args['value'] == 'slug' ) {
        // for multi select
