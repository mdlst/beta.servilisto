...
function ae_the_time($from)
{
    ...
    if (time() - $from > (7 * 24 * 60 * 60)) {
        ...
        return __('Ago', ET_DOMAIN) . ' ' . ae_human_time_diff($from);
    }
}
...
function et_load_mobile()
{
    ...
    // var_dump($detector->isMobile());
    // var_dump($detector->isTablet());
...
if (!function_exists('et_get_page_link')) {
    /*01_11_2016 (M) Id: 1 S */
    function et_get_page_link($pages, $params = array(), $create = false)
    {
        /*01_11_2016 (M) Id : 1 E */
        $page_args = array(
            'post_title' => '',
...
function et_the_time($from)
{
    ...
    //
    if (time() - $from > (7 * 24 * 60 * 60)) {
        return sprintf(__('el %s', ET_DOMAIN), date_i18n(get_option('date_format'), $from, true));
    } else {
...
if (!function_exists('ae_gg_recaptcha')) {
    ...
    function ae_gg_recaptcha()
    {
        if (ae_get_option('gg_captcha', false) && ae_get_option('gg_site_key')) {
            ?>
            <div id="recaptcha_register" class="g-recaptcha" data-callback="captchaCallbackRegister"></div>
            <input type="hidden" class="recaptcha_register_1 required" name="recaptcha_register_1" id="recaptcha_register_1">
          <?php
        }
    }
}

if (!function_exists('ae_gg_recaptcha2')) {
    /**
     *
     * Print out google captcha no-captcha
     * @param null
     ...
     *
     **/
    function ae_gg_recaptcha2()
    {
        if (ae_get_option('gg_captcha', false) && ae_get_option('gg_site_key')) {
            ?>
            <div id="recaptcha_publica_tu_anuncio" class="g-recaptcha" data-callback="captchaCallbackPublishPlace"></div>
            <input type="hidden" class="recaptcha_publica_tu_anuncio_1 required" name="recaptcha_publica_tu_anuncio_1" id="recaptcha_publica_tu_anuncio_1">
            <?php
        }
    }
}
