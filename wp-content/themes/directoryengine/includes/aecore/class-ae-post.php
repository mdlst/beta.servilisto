...
class AE_Posts
{
    ...
    public function insert($args)
    {
        ...
        $args = apply_filters('ae_pre_insert_' . $this->post_type, $args);  // aqui las validaciones
        ...
        $current_user->remove_role('user');
        $current_user->remove_role('advertiser');

        if ($result->et_payment_package == "001") {
            if (!in_array('author', (array)$user->roles)) {
                $current_user->add_role('author');
            }
        } else {
            if (!in_array('author', (array)$user->roles)) {
                $current_user->add_role('advertiser');
            }
        }
    ...
    public function update($args)
    {
        ...
        $opentime1 = $_POST['open_time'];
        $closetime1 = $_POST['close_time'];
        $opentime2 = $_POST['open_time_2'];
        $closetime2 = $_POST['close_time_2'];
        ...
        foreach ($_POST['serve_time'] as $key => $days) {
            if ($days['open_time'] != "" || $days['close_time'] != "" || $days['open_time_2'] != "" || $days['close_time_2'] != "")
                $weeks[] = $key;
        }

        $rec['calendar_id'] = $_POST['DOPBSP-calendar-ID'];
        ...
        $rec['days'] = implode(",", $weeks);
        $rec['data'] = implode(",", @$arr);
        //$rec['data'] = $opentime1.'-'.$closetime1.','.$opentime2.'-'.$closetime2;
        $rec['status'] = 1;
        $rec['price'] = $_POST['hourly_rate1'];
        $table = $wpdb->prefix . 'dopbsp_days';
        $condition['calendar_id'] = $_POST['DOPBSP-calendar-ID'];
        $condition['status'] = 1;
        $wpdb->update($table, $rec, $condition);
        /* 03_12_2016 Id : 1 E*/
    ...
    public function nearbyPost($args)
    {
        ...
        if (isset($args['location']) && $args['location'] == "cerca-de-ti" && $_COOKIE["current_user_province"]) {
            $args['location'] = $_COOKIE["current_user_province"];
        }

        if (isset($args['location']) && $args['location'] != "cerca-de-ti") {
            unset($args["radius"]);
            unset($args["near_lat"]);
            unset($args["near_lng"]);
        }
        ...
        // get nearby post id
        if (isset($args['near_lat']) && $args['near_lat'] && $args['radius'] && isset($args['location']) && $args['location'] == "cerca-de-ti" ) {


            $km = $args['radius']; // en km
            ...
            $sql = "select ID , $calc as dis
                        ...
                        $calc <= $km GROUP BY ID order by dis";
